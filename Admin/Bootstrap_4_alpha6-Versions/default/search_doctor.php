                ...
                <div>
                    <div class="card-box">
                        <div class="row">
                            <div class="col-12">
                                <div class="p-20">
                                    <form action="search_doctor.php" method="get" class="form-horizontal" name="fixedform">
                                        <label class="col-2 col-form-label">Enter Name/ Category: </label>
                                        <div class="col-10">
                                    ...
                                    if (isset($_GET['Find'])) {
                                        $find = $_GET['Find'];
                                        $query_doctor = "select * from doctor where name like '%$find%'";
                                        $query_doctor_run = mysqli_query($link, $query_doctor);
                                        if(mysqli_num_rows($query_doctor_run)!=0) {
                                            while ($query_doctor_row = mysqli_fetch_assoc($query_doctor_run)) {
                                                $name = $query_doctor_row['name'];
                                                $_SESSION['doc_id']=$query_doctor_row['doctor_ID'];
                                                echo '<a href="viewDoctor.php">'."<br>"."<h5>".$name."</h5>".'</a>';
                                            }
                                        }else {
                                            $query_doctor_1 = "select * from doctor where category ='$find'";
                                            $query_doctor_run_1 = mysqli_query($link, $query_doctor_1);
                                            if (mysqli_num_rows($query_doctor_run_1) != 0) {
                                                while ($query_doctor_row = mysqli_fetch_assoc($query_doctor_run_1)) {
                                                    $name = $query_doctor_row['name'];
                                                    $_SESSION['doc_id'] = $query_doctor_row['doctor_ID'];
                                                    echo '<a href="viewDoctor.php">' . "<br>" . "<h5>" . $name . "</h5>" . '</a>';
                                                }
